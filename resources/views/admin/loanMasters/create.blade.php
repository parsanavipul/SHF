...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.loanMaster.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.loan-masters.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subproduct') ? 'is-invalid' : '' }}" name="subproduct_id" id="subproduct_id" required>
                    @foreach($subproducts as $id => $entry)
                        <option value="{{ $id }}" {{ old('subproduct_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('loan_account_no') ? 'is-invalid' : '' }}" type="text" name="loan_account_no" id="loan_account_no" value="{{ old('loan_account_no', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('loan_tenure') ? 'is-invalid' : '' }}" type="number" name="loan_tenure" id="loan_tenure" value="{{ old('loan_tenure', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_self_connector') ? 'is-invalid' : '' }}" name="is_self_connector" id="is_self_connector" required>
                    ...
                    @foreach(App\Models\LoanMaster::IS_SELF_CONNECTOR_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_self_connector', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dme_1') ? 'is-invalid' : '' }}" name="dme_1_id" id="dme_1_id" required>
                    @foreach($dme_1s as $id => $entry)
                        <option value="{{ $id }}" {{ old('dme_1_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('dme_1'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dme_2') ? 'is-invalid' : '' }}" name="dme_2_id" id="dme_2_id" required>
                    @foreach($dme_2s as $id => $entry)
                        <option value="{{ $id }}" {{ old('dme_2_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('dme_2'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dme_3') ? 'is-invalid' : '' }}" name="dme_3_id" id="dme_3_id">
                    @foreach($dme_3s as $id => $entry)
                        <option value="{{ $id }}" {{ old('dme_3_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('dme_3'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('sanctioned_date') ? 'is-invalid' : '' }}" type="text" name="sanctioned_date" id="sanctioned_date" value="{{ old('sanctioned_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sanctioned_amount') ? 'is-invalid' : '' }}" type="number" name="sanctioned_amount" id="sanctioned_amount" value="{{ old('sanctioned_amount', '') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('disbursement_date') ? 'is-invalid' : '' }}" type="text" name="disbursement_date" id="disbursement_date" value="{{ old('disbursement_date') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('disbursement_amount') ? 'is-invalid' : '' }}" type="number" name="disbursement_amount" id="disbursement_amount" value="{{ old('disbursement_amount', '') }}" step="0.01">
