    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.customers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('reference_by') ? 'is-invalid' : '' }}" name="reference_by_id" id="reference_by_id" required>
                    @foreach($reference_bies as $id => $entry)
                        <option value="{{ $id }}" {{ old('reference_by_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('pan_card_no') ? 'is-invalid' : '' }}" type="text" name="pan_card_no" id="pan_card_no" value="{{ old('pan_card_no', '') }}">
