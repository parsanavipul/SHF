...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-masters.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent_product') ? 'is-invalid' : '' }}" name="parent_product_id" id="parent_product_id">
                    @foreach($parent_products as $id => $entry)
                        <option value="{{ $id }}" {{ old('parent_product_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('payout') ? 'is-invalid' : '' }}" type="number" name="payout" id="payout" value="{{ old('payout', '') }}" step="0.01" required max="100">
